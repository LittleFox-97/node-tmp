...
module.exports.assertPostfix = function assertPostfix(name, postfix) {
  assert.equal(name.slice(name.length - postfix.length), postfix, 'should have the provided postfix')
}
...
module.exports.assertProperResult = function assertProperResult(result, withfd) {
  ...
  if (withfd) {
    assert.ok(result.fd, 'should have an fd')
  } else {
    assert.strictEqual(result.fd, undefined, 'should not have a file descriptor (fd)')
  }
...
module.exports.assertExists = function assertExists(name, isfile) {
  ...
  const stat = fs.statSync(name)
  if (isfile) {
    assert.ok(stat.isFile(), `${name} should be a file`)
  } else {
    assert.ok(stat.isDirectory(), `${name} should be a directory`)
  }
}
